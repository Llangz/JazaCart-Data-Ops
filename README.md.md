...
/sql: Contains the Schema definitions, ETL logic, and Audit scripts.
/docs: View the Root Cause Analysis (RCA) on duplicate orders and the Onboarding Guide.
...
Identify: Query ops.etl_run_log for the exact error message.
Investigate: Run the Audit Query to isolate specific records in staging.orders.
Contain: If critical, disable Power BI refreshes to prevent "Garbage In, Garbage Out."
Resolve: Correct the record or update the validation rule in the stored procedure.
Communicate: Notify #data-alerts on Slack/Teams; update stakeholders if delay > 2 hours.
